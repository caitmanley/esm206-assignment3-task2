...
mpg_make_model <- mpg %>% # Start with mpg
  unite(col = make_model, # Create a new column called 'make_model'
        ...
        sep = "_" # Puts "HEY!" between the pieces (default is underscore)
        ) 
...
gasol_hookshots <- lakers %>% 
  ...
  mutate(new_dates = ymd(date)) %>% 
  mutate(new_month = month(new_dates)) %>% 
  select("date", "player", "type", "new_dates", "new_month")

hookshots_by_month <- gasol_hookshots  %>% 
  group_by(new_month) %>% 
  count()
